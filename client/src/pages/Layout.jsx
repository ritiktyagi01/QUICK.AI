...
const Layout = () => {
  ...
  return user ? (
    ...
    <nav className='flex justify-between items-center px-8 min-h-14 w-full border-b border-gray-500'>
       ...
       {sidebar ? <X onClick={() => setSideBar(false)} className='w-6 h-6 text-gray-600 sm:hidden' /> : <Menu onClick={() => setSideBar(true)} className='w-6 h-6 cursor-pointer sm:hidden text-gray-600 ' />} 
        </nav> <div className=' flex-1 w-full flex h-[calc(100vh-64px)] '>
